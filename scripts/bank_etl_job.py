...
refined_df=filtereddf.withColumn("customer_dob",F.col("CustomerDOB").cast(dataType="Date")) \
            .withColumn("transaction_date",F.col("TransactionDate").cast(dataType="Date"))

refined_df=refined_df.withColumn("Age",F.date_diff(F.curdate(),F.col("customer_dob"))//365.25)



categorized_df=refined_df.withColumn("Segment",category_udf(F.col("Age"),F.col("CustomerBalance")))

categorized_df.withColumn("transaction_frequency",F.lag("TransactionDate").over(winowSpec)) \
                    .withColumn("total_spent",F.sum("TransactionAmount").over(winowSpec))
categorized_df.write.parquet(vars["destination_path"],"overwrite",partitionBy=["CustLocation","Segment"])
