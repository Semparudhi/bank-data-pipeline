...
# --- Filter + format
df = df.filter((F.col("TransactionAmount (INR)") >= 0) & (F.col("TransactionAmount (INR)").isNotNull()))
df = df.withColumn("customer_dob", F.to_date("CustomerDOB", "yyyy-MM-dd")) \
       .withColumn("transaction_date", F.to_date("TransactionDate", "yyyy-MM-dd")) \
       .withColumn("Age", (F.datediff(F.current_date(), "customer_dob") / 365.25).cast("int"))
...
df = df.withColumn("days_between_txns", F.datediff("transaction_date", F.lag("transaction_date", 1).over(windowSpec))) \
       .withColumn("total_spent", F.sum("TransactionAmount (INR)").over(windowSpec))

# --- Write output partitioned by location & segment
df.write.mode("overwrite") \
    .partitionBy("CustLocation", "Segment") \
    .parquet(output_path)
